...
import processwire from '../src'
import { resolvePageComponent } from '../src/inertia-helpers';

describe('processwire-vite-plugin', () => {
    afterEach(() => {
        vi.clearAllMocks()
    })
    ...
    it('handles missing configuration', () => {
        ...
        /* @ts-ignore */
        expect(() => processwire())
            .toThrowError('processwire-vite-plugin: missing configuration.');

        /* eslint-disable-next-line @typescript-eslint/ban-ts-comment */
        /* @ts-ignore */
        expect(() => processwire({}))
            .toThrowError('processwire-vite-plugin: missing configuration for "input".');
    })

    it('accepts a single input', () => {
        const plugin = processwire('resources/js/app.js')[0]
    ...
    it('uses the default entry point when ssr entry point is not provided', () => {
        ...
        const plugin = processwire('resources/js/ssr.js')[0]
    ...
    it('prevents setting an empty publicDirectory', () => {
        expect(() => processwire({ input: 'resources/js/app.js', publicDirectory: '' })[0])
            .toThrowError('publicDirectory must be a subdirectory');
    })

    it('prevents setting an empty buildDirectory', () => {
        expect(() => processwire({ input: 'resources/js/app.js', buildDirectory: '' })[0])
    ...
    it('appends an Alias object when using an alias array', () => {
        const plugin = processwire('js/app.js')[0]
    ...
    it('prevents the Inertia helpers from being externalized', () => {
        ...
        /* @ts-ignore */
        expect(noSsrConfig.ssr.noExternal).toEqual(['processwire-vite-plugin'])
    ...
    it('configures full reload with routes and views when refresh is true', () => {
        ...
        /** @ts-ignore */
        expect(plugins[1].__processwire_plugin_config).toEqual({
            paths: [
              "resources/js/**",
              "resources/lang/**",
              "site/templates/**",
    ...
    it('configures full reload when refresh is an array of complete configurations to proxy', () => {
        ...
        /** @ts-ignore */
        expect(plugins[2].__processwire_plugin_config).toEqual({
            paths: ['another/to/watch/**'],
            config: { delay: 123 }
        })
    })
