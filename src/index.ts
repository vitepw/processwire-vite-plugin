...
interface PluginConfig {
  ...
  /**
   * ProcessWire's public directory.
   *
   * @default 'public'
   */
  ...
  /**
   * Configuration for performing full page refresh on php templates (or other) file changes.
   *
...
interface ProcessWirePlugin extends Plugin {
...
/**
 * ProcessWire plugin for Vite.
 *
 * @param config - A config object or relative path(s) of the scripts to be compiled.
 */
export default function processwire(
  config: string | string[] | PluginConfig
): [ProcessWirePlugin, ...Plugin[]] {
  const pluginConfig = resolvePluginConfig(config);

  return [
    resolveProcessWirePlugin(pluginConfig),
    ...(resolveFullReloadConfig(pluginConfig) as Plugin[]),
  ];
}

/**
 * Resolve the ProcessWire Plugin configuration.
 */
function resolveProcessWirePlugin(
  ...
  return {
    ...
    config: (userConfig, { command, mode }) => {
      ...
      return {
        ...
        server: {
          origin:
            userConfig.server?.origin ?? "__processwire_vite_placeholder__",
          ...(serverConfig
            ? {
    ...
    transform(code) {
      if (resolvedConfig.command === "serve") {
        code = code.replace(
          /__processwire_vite_placeholder__/g,
          viteDevServerUrl
        );
        ...
        if (isAddressInfo(address)) {
          ...
          setTimeout(() => {
            server.config.logger.info(
              `\n  ${colors.blue(
                `${colors.bold("🌀 PROCESSWIRE")} ${processwireVersion()}`
...
function ensureCommandShouldRunInEnvironment(
  ...
  if (command === "build" || env.PROCESSWIRE_BYPASS_ENV_CHECK === "1") {
    return;
  }

  if (typeof env.CI !== "undefined") {
    throw Error(
      "You should not run the Vite HMR server in CI environments. You should build your assets for production instead. To disable this ENV check you may set PROCESSWIRE_BYPASS_ENV_CHECK=1"
    );
  }
}

/**
 * The version of ProcessWire being run.
 */
function processwireVersion(): string {
  try {
    ...
    return (
      composer.packages?.find(
        (composerPackage: { name: string }) =>
          composerPackage.name === "processwire/processwire"
      )?.version ?? ""
    );
  } catch {
    return "";
...
/**
 * The version of the ProcessWire Vite plugin being run.
 */
...
function resolvePluginConfig(
  ...
  if (typeof config === "undefined") {
    throw new Error("processwire-vite-plugin: missing configuration.");
  }
  ...
  if (typeof config.input === "undefined") {
    throw new Error(
      'processwire-vite-plugin: missing configuration for "input".'
    );
  }
  ...
  if (typeof config.publicDirectory === "string") {
    ...
    if (config.publicDirectory === "") {
      throw new Error(
        "processwire-vite-plugin: publicDirectory must be a subdirectory. E.g. 'public'."
      );
    }
  }
  ...
  if (typeof config.buildDirectory === "string") {
    ...
    if (config.buildDirectory === "") {
      throw new Error(
        "processwire-vite-plugin: buildDirectory must be a subdirectory. E.g. 'build'."
      );
    }
  }
...
function resolveFullReloadConfig({
  ...
  return (config as RefreshConfig[]).flatMap((c) => {
    ...
    /** @ts-ignore */
    plugin.__processwire_plugin_config = c;
...
function noExternalInertiaHelpers(
  ...
  const pluginNoExternal = ["processwire-vite-plugin"];
